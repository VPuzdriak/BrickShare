...
episode for why, and episode 27 for where it goes. The argument is the same one, twice.
...
So what lands is not a column. It is the thing that makes the first money column, in episode 27,
...
- **No `catalog_sets`, no `set_id`, no set-number constraint.** The domain has no `CatalogSet` type,
  ...
  honest description of a domain that has no set in it. Episode 27 creates sets, and brings the
