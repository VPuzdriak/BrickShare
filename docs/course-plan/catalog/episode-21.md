...
fourth line there would work today and be the wrong habit by episode 35, so the shape goes in first.
...
episode 35 adds available copy count and a starting price to the read model, numbers that are
...
`GET /api/v1/sets/{id}` — **and that route does not exist until episode 35**, so following it right
now returns 404.
...
**That works, and it should not.** Nothing in this service knows what a staff member is. Episode 38
...
**No OpenAPI document.** The endpoint exists and nothing describes it. That waits until episode 32,
...
| Following that `Location` | **`404` until episode 35.** Expected, and the reason is in step 3 |
