...
fourth line there would work today and be the wrong habit by episode 33, so the shape goes in first.
...
episode 33 adds available copy count and a starting price to the read model, numbers that are
...
`GET /api/v1/sets/{id}` — **and that route does not exist until episode 33**, so following it right
now returns 404.
...
**That works, and it should not.** Nothing in this service knows what a staff member is. Episode 36
...
**No OpenAPI document.** The endpoint exists and nothing describes it. That waits until episode 30,
...
| Following that `Location` | **`404` until episode 33.** Expected, and the reason is in step 3 |
